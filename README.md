...
| 13 | 🔴 **9.1** | `ueberauth_apple` · Hex/Elixir | "Sign in with Apple" id_token accepted with no `aud`/`iss`/`exp`/`nonce` validation → audience-confusion / replay → ATO `CWE-345` · CVSS v4.0 9.1 (v3.1 = 7.4 High) | [GHSA-pxx8-68pc-p9mr](https://github.com/ueberauth/ueberauth_apple/security/advisories/GHSA-pxx8-68pc-p9mr) | 🔄 pending |
...
| 18 | 🟠 **7.5** | `jose_plus` · Dart/pub.dev | JWS signature forgery — the verifier trusts an attacker-supplied embedded `jwk` header as the signing key (no binding to a trusted key) → forge any signed token; same defect as the parent `jose` (CVE-2026-34240) `CWE-347` · CVSS v4.0 8.7 High | [GHSA-vm9r-h74p-hg97](https://github.com/Bdaya-Dev/jose/security/advisories/GHSA-vm9r-h74p-hg97) | 🔄 pending |
...
<sub>Reported via GitHub Security Advisory (credit accepted), except rows 25 / 29 / 30 (MITRE-routed, no PVR). CVE IDs land on publication. The headline severity is the as-filed CVSS v3.1 Base; the `CVSS v4.0 …` note is the FIRST.org v4.0 Base recompute of the same vulnerability semantics (calculator-derived, cross-checked between two independent implementations). The v4.0 band can differ from the v3.1 headline — usually higher, since 4.0 drops the v3.1 Scope discount (so a couple of network/unauth confidentiality+integrity bugs land in Critical), occasionally lower for blind / subsequent-scope SSRF.</sub>

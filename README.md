...
<sub>Reported via GitHub Security Advisory (credit accepted), except rows 25 / 29 / 30 (MITRE-routed, no PVR). CVE IDs land on publication. The headline severity is the as-filed CVSS v3.1 Base; where shown, `CVSS v4.0 …` is the FIRST.org v4.0 Base recompute (same vulnerability semantics — 4.0 scores network/unauth single-High-impact bugs ~1–1.5 pts higher because it drops the v3.1 Scope discount).</sub>
